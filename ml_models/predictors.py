...
def find_model_directories():
    """
    Find model directories - models are organized in ml_models/*_prediction_package
    """
    # Get the project root directory
    project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    
    # Define organized model paths (using prediction packages)
    gender_model_dir = os.path.join(project_root, "ml_models", "gender_prediction_package")
    region_model_dir = os.path.join(project_root, "ml_models", "region_prediction_package")
    
    # Verify directories exist and contain required files
    found_sex_dir = None
    found_ancestry_dir = None

    # Check Gender Model directory (gender_prediction_package)
    if os.path.exists(gender_model_dir):
        ...
        if has_model:
            ...
            print(f"[OK] Gender model package found: {gender_model_dir}")
        else:
            print(f"[WARN] Gender model package exists but no model files found: {gender_model_dir}")
    else:
        print(f"[ERROR] Gender model package not found: {gender_model_dir}")

    # Check Region/Ancestry Model directory (region_prediction_package)
    if os.path.exists(region_model_dir):
        ...
        if os.path.exists(model_file) and os.path.exists(encoder_file):
            ...
            print(f"[OK] Region/Ancestry model package found: {region_model_dir}")
        else:
            print(f"[WARN] Region model package exists but missing required files: {region_model_dir}")
    else:
        print(f"[ERROR] Region model package not found: {region_model_dir}")
